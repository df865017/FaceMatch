...
void Mouth::on_mouse(int event,int x,int y,int flags,void *ustc)
{
		...
		{  
			...
			pre_pt = Point(x,y);  
			putText(Pmouth->mat_public,temp,pre_pt,FONT_HERSHEY_SIMPLEX,0.3,Scalar(0,0,255,255),1,8,false);//在窗口上显示坐标  
			circle(Pmouth->mat_public,pre_pt,2,Scalar(255,0,0,0),CV_FILLED,CV_AA,0);						//划点  
